<template>
  <div class="page-container">
    ...
    <el-dialog v-model="applyDialogVisible" :title="userStore.isAdmin || userStore.isHR ? '新增出差' : '申请出差'" width="500px">
      <el-form ref="applyFormRef" :model="applyForm" label-width="100px" :rules="applyRules">
        <el-form-item label="出差地点" prop="destination">
          <el-input v-model="applyForm.destination" placeholder="请输入出差地点" />
        </el-form-item>
        <el-form-item label="开始日期" prop="startDate">
          <el-date-picker v-model="applyForm.startDate" type="date" placeholder="选择日期" value-format="YYYY-MM-DD" style="width: 100%" />
        </el-form-item>
        <el-form-item label="结束日期" prop="endDate">
          <el-date-picker v-model="applyForm.endDate" type="date" placeholder="选择日期" value-format="YYYY-MM-DD" style="width: 100%" />
        </el-form-item>
...
import { getBusinessTripList, getMyBusinessTripList, createBusinessTrip, approveBusinessTrip } from '@/api/businessTrip'
...
const applyRules = {
  ...
  reason: [{ required: true, message: '请输入出差原因', trigger: 'blur' }]
}
...
const loadData = async () => {
  ...
  try {
    ...
    if (userStore.isAdmin || userStore.isHR) {
      res = await getBusinessTripList(params)
    } else {
      res = await getMyBusinessTripList(params)
    }
...
const handleApproveSubmit = async () => {
  ...
  try {
    await approveBusinessTrip(approveForm.id, {
...
const handleExport = () => {
  const headers = ['员工姓名', '出差地点', '开始日期', '结束日期', '出差原因', '状态', '审批意见']
  ...
  link.download = `出差记录_${new Date().toLocaleDateString()}.csv`
  link.click()
